...
The system operates in a continuous loop, cycling through all 8 exhibits automatically:
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   Relay 1 ON ──► (10 sec) ──► OFF                                  │
│                                 │                                   │
│   Relay 2 ON ◄──────────────────┘                                  │
│         │                                                           │
│      (10 sec)                                                       │
│         │                                                           │
│        OFF ──► Relay 3 ON ──► ... ──► Relay 8 ON ──► OFF           │
│                                                                     │
│   ┌─────────────────────────────────────────────────────────────┐  │
│   │  After Relay 8 finishes → Cycle restarts from Relay 1       │  │
│   └─────────────────────────────────────────────────────────────┘  │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
**Cycle Rules:**
1. Only ONE relay is active at a time
2. Each relay stays ON for its configured duration
3. When one relay turns OFF, the next one turns ON immediately
4. After Relay 8 completes, the cycle restarts from Relay 1
5. The cycle runs forever until interrupted by a button press
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│   Visitor presses "Hair Dryer" button                              │
│                    │                                                │
│                    ▼                                                │
│   ┌────────────────────────────────┐                               │
│   │  1. Current relay turns OFF    │                               │
│   │  2. Auto-cycle PAUSES          │                               │
│   │  3. Hair Dryer relay turns ON  │                               │
│   └────────────────────────────────┘                               │
│                    │                                                │
│                    ▼                                                │
│           (7 seconds pass)                                          │
...
| Action | Result |
|--------|--------|
| Press any button | Interrupts current relay, activates selected relay |
| Press same button twice | Relay stays ON for its full duration again |
| Press during night mode | Only Lava Lamp button works (others ignored) |
| No button pressed | Auto-cycle continues normally |

---

### Night Mode Behavior

During configured night hours (default: midnight to 6 AM):
...
│                    ▼                                                │
│   ┌────────────────────────────────────────────────────┐           │
│   │  • Only Lava Lamp (Relay 4) cycles                 │           │
│   │  • All other relays are skipped                    │           │
│   │  • Web buttons for other relays are disabled       │           │
│   │  • Saves energy during off-hours                   │           │
│   └────────────────────────────────────────────────────┘           │
│                                                                     │
│   6:01 AM ─────────────────────────────────── 11:59 PM             │
│      │                                            │                 │
│      └──── NORMAL MODE (all relays cycle) ────────┘                 │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
The web interface shows button states with colors:

| Button Color | Meaning |
|--------------|---------|
| 🟢 Green | Relay is currently ON |
| ⚫ Gray/Default | Relay is OFF, ready to activate |
| 🔴 Red (disabled) | Night mode - button unavailable |
